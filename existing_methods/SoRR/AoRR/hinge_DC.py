...
class hinge_TOPK_DC:
    def __init__(self, lr=0.01, num_iter=100, inner_iter=1000, fit_intercept=True, verbose=True, k_value=5, k2_value = 4, early_stop=20, seed = 1234,dataname='real', Model_name='LogisticRegression',train_loss=None):
        self.lr = lr
        ...
        for i in range(self.num_iter):
            ...
            for j in range(self.inner_iter):
                ...
                if accuracy >= acc_inital:
                    np.save(pwd+'/immedia_parameter/{}_{}_theta.npy'.format(self.dataname,self.Model_name), self.theta)
            ...
            self.theta = np.load(pwd+'/immedia_parameter/{}_{}_theta.npy'.format(self.dataname,self.Model_name))
            if (self.verbose == True and i % 1 == 0):
                ...
                accuracy2_test = metrics.classification_report(y_test, outer_test, digits=3, output_dict=True)[
                    'accuracy']
                print("outer iter:", i, "train loss:", self.train_loss(torch.from_numpy(self.theta).double().reshape(-1,1)))
            ...
            self.t_array.append(time.time()-t_start)
